import SwiperRompeHielo from "./swiperRompeHielo";

const modalRompeHielo = ({ setModalOn, setChoice, numeroSlide }) => {
  const handleOKClick = () => {
    setChoice(true);
    setModalOn(false);
  };
  ...
  return (
    <div className="mx-auto fixed inset-0 z-50 bg-gradient-to-r from-indigo-900 via-fuchsia-900 to-indigo-900">
      <div className="relative h-screen justify-center items-center">
        ...
        <SwiperRompeHielo numeroSlide={numeroSlide} />
      </div>
    </div>
  );
};

export default modalRompeHielo;
